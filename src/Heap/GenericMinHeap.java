...
public class GenericMinHeap<E extends Comparable<E>> implements Heap<E> {
    ...
    private void maxHeapify(int i) {
        ...
        if(leftChildren < this.size - 1)
            if(((E)this.array[leftChildren]).compareTo((E)this.array[smaller]) < 0)
                smaller = leftChildren;
            
        if(rightChildren < this.size - 1)
            if(((E)this.array[rightChildren]).compareTo((E)this.array[smaller]) < 0)
                smaller = rightChildren;

        if (i != this.size - 1 && smaller != i) {
            Object buffer = this.array[smaller];
